...
//
//  total           → admin/super_admin: veem e editam QUALQUER membro (inclui
//                    e-mail, equipe, nível/status via /usuarios).
//  equipe          → líder: vê e edita SÓ os membros da própria equipe, e
//                    apenas os dados de PERFIL (nome, celular, nascimento,
//                    observação, foto) — nunca e-mail, equipe, nível ou status.
//  somente_leitura → monitor: vê a lista completa, mas nunca edita ninguém.
...
export type AcessoMembros = "total" | "equipe" | "somente_leitura" | "nenhum";
...
export function acessoMembros(usuario: UsuarioAtual): AcessoMembros {
  ...
  if (can(usuario.nivelAcesso, "ver_membros_todos")) return "somente_leitura";
  return "nenhum";
}
...
// equipe por acessoMembros() = "equipe"; aqui só confirmamos o vínculo. O
// monitor ("somente_leitura") nunca edita ninguém.
export function podeEditarMembroAlvo(
  usuario: UsuarioAtual,
  alvo: { nivelAcesso: string; equipeId: string | null },
): boolean {
  const acesso = acessoMembros(usuario);
  if (acesso === "nenhum" || acesso === "somente_leitura") return false;
